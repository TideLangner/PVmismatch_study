...
def plot_degraded(Imod_deg, Vmod_deg, Pmod_deg, Istr_deg, Vstr_deg, Pstr_deg, Isys_deg, Vsys_deg, Psys_deg, deg_label):
...
def plot_deg_vs_healthy_mods(Imod, Vmod, Pmod, Imod_deg, Vmod_deg, Pmod_deg, deg_label):
...
def plot_degradation_modes(all_modes):

    fig, (ax_iv, ax_pv) = plt.subplots(1, 2, figsize=(10, 6))

    # Plot limits
    Imax = max(np.max(s["Isys_deg"]) for s in all_modes)
    Vmax = max(np.max(s["Vsys_deg"]) for s in all_modes)
    Pmax = max(np.max(s["Psys_deg"]) for s in all_modes)
    ...
    for i, s in enumerate(all_modes):
        ax_iv.plot(s["Vsys_deg"], s["Isys_deg"], label=s["deg_label"], color=c[i])
        ax_pv.plot(s["Vsys_deg"], s["Psys_deg"], label=s["deg_label"], color=c[i])

    # IV subplot
    ax_iv.set_title("I–V Curves for All Degraded Systems")
    ax_iv.set_xlabel("Voltage [V]")
    ax_iv.set_xlim(0, 1.1 * Vmax)
    ax_iv.set_ylabel("Current [A]")
    ax_iv.set_ylim(0, 1.1 * Imax)
    ax_iv.grid(True, linestyle="--", alpha=0.4)
    ax_iv.legend()

    # PV subplot
    ax_pv.set_title(f"P–V Curves for All Degraded Systems")
    ax_pv.set_xlabel("Voltage [V]")
    ax_pv.set_xlim(0, 1.1 * Vmax)
    ax_pv.set_ylabel("Power [W]")
    ax_pv.set_ylim(0, 1.1 * Pmax)
    ax_pv.grid(True, linestyle="--", alpha=0.4)
    ax_pv.legend()
