...
def plot_mismatch_3d(k_mesh, set_mesh, z_mesh, z_mode="W", title=None, mode=1):
    ...
    if np.isfinite(z_mesh).any():
        ...
        ax.scatter([k_max], [y_max], [z_max], color="crimson", s=45, depthshade=True)
    ...
    if z_mode == "%":
        plt.savefig(save_dir / f"{mode}strs_mismatch_3d_surface_per.png", dpi=600)
        plt.savefig(save_dir / f"{mode}strs_mismatch_3d_surface_per.pdf")
    else:
        plt.savefig(save_dir / f"{mode}strs_mismatch_3d_surface_W.png", dpi=600)
        plt.savefig(save_dir / f"{mode}strs_mismatch_3d_surface_W.pdf")
